...
<div id="k2ModuleBox<?php echo $module->id; ?>" class="k2ItemsBlock<?php if ($params->get('moduleclass_sfx')) echo ' ' . $params->get('moduleclass_sfx'); ?>">
    ...
    <?php if (isset($items) && count($items)) : ?>
        ...
        foreach ($items as $key => $item) {
            $date[$key] = json_decode($item->extra_fields)[0]->value;
        }
        $category = array_column($items, 'extra_fields_search');
        array_multisort($category, SORT_DESC, $items);
        array_multisort($category, SORT_ASC, $items);
        ...
        <ul>
            <?php foreach ($items as $key => $item) :  ?>
                <li class="<?php echo ($key % 2) ? "odd" : "even";
                    ...
                    <?php print_r(json_decode($item->extra_fields)[0]->value); ?>
                    <?php print_r($item->extra_fields_search); ?>
